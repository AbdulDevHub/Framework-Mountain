...
import { getAIModelName } from "@/lib/openai";

const isOpenAI = !!process.env.OPENAI_API_KEY;
const aiKey = process.env.OPENAI_API_KEY || process.env.OPEN_ROUTER_API_KEY;
const innerModelName = getAIModelName();
...
const stagehand = new Stagehand({
  ...
  model: {
    modelName: `openai/${innerModelName}`,
    apiKey: aiKey!,
    ...(isOpenAI ? {} : { baseURL: "https://openrouter.ai/api/v1" }),
  },
  disablePino: true,
});
...
- Model is resolved dynamically via `getAIModelName()` — supports standard OpenAI `gpt-4o` or OpenRouter free models like `meta-llama/llama-3.3-70b-instruct:free`
...
```typescript
import { getOpenAIClient, getAIModelName } from "@/lib/openai";
...
- Model string is resolved dynamically using `getAIModelName()` — never hardcode model names
